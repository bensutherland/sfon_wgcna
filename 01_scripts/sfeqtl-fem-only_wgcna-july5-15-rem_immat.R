...
posit.fem.files2 <- which(fem.files.df$file.name %in% fem.files)
rownames(datExpr0.fem) <- fem.files.df$lib.ID[posit.fem.files2]
head(datExpr0.fem[1:5,1:5])
...
par(mfrow=c(1,1))
sampleTree.fem <- hclust(dist(datExpr0.fem), method = "average") # default dist metric = euclidean; hclust agglomeration is "average"
par(mar = c(0,4,2,0), cex = 0.6)
plot(sampleTree.fem, main = "Sample clustering to detect outliers", sub="", xlab="", cex.lab = 1.5,
...
cutline <- 80
#for cpm > 1
#cutline <- 160 # choose height at which to cut
...
keepSamples = (clust==1)
datExpr = datExpr0.fem[keepSamples, ] # subset the expression data to only keep samples in the main group
nGenes = ncol(datExpr)
...
colnames(traitData[, -c(1,3:5,7,8:10,26:27,30,41:45)]) #NOTE: what is this d.weight trait..?
allTraits <- traitData[, -c(1,3:5,8:10,26:27,30,41:45)] # remove unneeded columns
names(allTraits)
...
labeledHeatmap(Matrix = moduleTraitCor,
               ...
               yLabels = names(datMEs),
               ySymbols = names(datMEs),
               colorLabels = FALSE,
...
# Correlate ME to Trait
par(mar=c(3,3,3,3), mfrow =c(1,1))
names(datMEs)
plot(datMEs$MEsalmon ~ datTraits$osmo.delta,
     ylab = "Mod Eigengene salmon", xlab = "Osmolarity delta", las =1)
mod <- lm(datMEs$MEsalmon ~ datTraits$osmo.delta)
summary(mod)
...
modNames <- substring(names(datMEs), 3) # obtain names (colors) of the modules
...
# write out results
write.csv(geneInfo0, file = "geneInfo0_unsigned_network.csv")
